...
[sarkhibot.io](https://sarkhibot.io) | [npm](https://www.npmjs.com/package/sarkhibot) | [GitHub](https://github.com/SarkhiAgent/sarkhibot)

An AI-powered Telegram assistant that runs a multi-agent swarm on your machine. Send a message and SarkhiBot dispatches specialized AI workers that write code, run commands, open pull requests, manage servers, and browse the web — all in parallel, all from Telegram.
...
You (Telegram) → Orchestrator (your chosen model)
     ...
     ┌──────────┬───────┼────────┬──────────┬──────────┐
     ↓          ↓       ↓        ↓          ↓          ↓
  Coding    Browser  System   DevOps    Research     Data    Creative   Social
...
- **Multi-agent swarm** — orchestrator + seven worker types (coding, browser, system, devops, research, data, creative) running in parallel.
- **Multi-model** — Anthropic, OpenAI, Google Gemini, and Groq. Switch anytime with `/brain` or `/orchestrator`.
- **75+ tools** — shell, files, Git, GitHub PRs, Docker, Puppeteer browsing, JIRA, system monitoring, networking, Claude Code, weather, translation, crypto, calculator, hashing, datetime, JSON/regex, DNS, HTTP client, cron, archives, and more.
...
- **Productivity** — built-in todo list, reminders, notes, code snippets, backup/restore, conversation export.
- **Utilities** — weather, translation, crypto prices, calculator, unit conversion, world clocks, JSON/regex tools.
- **Observability** — health monitoring, usage analytics, latency checks, rate limiting.
- **Security** — user allowlist, blocked paths, dangerous-op confirmation, audit logging, secret redaction, job timeouts.

## Quick Start

```bash
npm install -g sarkhibot
sarkhibot
```

On first run, SarkhiBot walks you through picking a provider, entering API keys, and setting up your Telegram bot token. Config is saved to `~/.sarkhibot/`.
...
| **System** | shell, files, process, monitor, network, utility, devtools | OS tasks, monitoring, DNS, HTTP, archives |
| **DevOps** | shell, files, Docker, process, monitor, network, git | Deployment, containers, infrastructure |
| **Research** | web search, browse, weather, translate, crypto | Deep web research and analysis |
| **Data** | JSON, regex, text, calculator, encoding, datetime, cron | Data processing, analysis, transformation |
| **Creative** | browse, translate, files | Content generation, writing, translation |
| **Social** | LinkedIn, X (Twitter) | Social media posting and engagement |

## Configuration

Config auto-detected from `./config.yaml` or `~/.sarkhibot/config.yaml`. Environment variables go in `.env` or `~/.sarkhibot/.env`.
...
See the [full config reference](https://github.com/SarkhiAgent/sarkhibot/blob/main/config.yaml) for all options.
...
Both the orchestrator and workers are configurable — use any supported provider and model. All persistent data lives in `~/.sarkhibot/`.

> **WARNING:** SarkhiBot has full access to your operating system. Only run it on machines you own and control. Always configure `allowed_users` in production.
