...
const SARKHIBOT_DIR = join(homedir(), '.sarkhibot');
const CHARACTERS_DIR = join(SARKHIBOT_DIR, 'characters');
const REGISTRY_FILE = join(CHARACTERS_DIR, 'registry.json');

const DEFAULT_REGISTRY = {
  activeCharacterId: 'sarkhi',
  characters: {},
};
...
export class CharacterManager {
  ...
  _load() {
    ...
    const legacySelfDir = join(SARKHIBOT_DIR, 'self');
    if (existsSync(legacySelfDir)) {
      // Existing installation — migrate to sarkhi character
      logger.info('[CharacterManager] Legacy data detected — migrating to sarkhi character');
      this._migrateToSarkhi();
    } else {
  ...
  _migrateToSarkhi() {
    const logger = getLogger();
    const sarkhiDir = join(CHARACTERS_DIR, 'sarkhi');
    mkdirSync(sarkhiDir, { recursive: true });

    // Copy self/ → characters/sarkhi/self/
    const legacySelfDir = join(SARKHIBOT_DIR, 'self');
    const targetSelfDir = join(sarkhiDir, 'self');
    if (existsSync(legacySelfDir) && !existsSync(targetSelfDir)) {
      cpSync(legacySelfDir, targetSelfDir, { recursive: true });
      logger.info('[CharacterManager] Migrated self/ → characters/sarkhi/self/');
    }

    // Copy life/ → characters/sarkhi/life/
    const legacyLifeDir = join(SARKHIBOT_DIR, 'life');
    const targetLifeDir = join(sarkhiDir, 'life');
    if (existsSync(legacyLifeDir) && !existsSync(targetLifeDir)) {
      cpSync(legacyLifeDir, targetLifeDir, { recursive: true });
      logger.info('[CharacterManager] Migrated life/ → characters/sarkhi/life/');
    }

    // Copy conversations.json → characters/sarkhi/conversations.json
    // Re-key all conversation and skill entries with "sarkhi:" prefix
    // so they match the new _chatKey() scoping in agent.js
    const legacyConvFile = join(SARKHIBOT_DIR, 'conversations.json');
    const targetConvFile = join(sarkhiDir, 'conversations.json');
    if (existsSync(legacyConvFile) && !existsSync(targetConvFile)) {
      try {
        ...
        for (const [key, value] of Object.entries(raw)) {
          if (key === '_skills') {
            // Re-key skills: "12345" → "sarkhi:12345"
            const migratedSkills = {};
            for (const [skillKey, skillId] of Object.entries(value)) {
              const newKey = skillKey.startsWith('sarkhi:') || skillKey.startsWith('__life__') ? skillKey : `sarkhi:${skillKey}`;
              migratedSkills[newKey] = skillId;
            }
            migrated._skills = migratedSkills;
          } else if (key.startsWith('__life__')) {
            // Re-key life engine chat: "__life__" → "__life__:sarkhi"
            migrated[key === '__life__' ? '__life__:sarkhi' : key] = value;
          } else {
            // Re-key user chats: "12345" → "sarkhi:12345"
            const newKey = key.startsWith('sarkhi:') ? key : `sarkhi:${key}`;
            migrated[newKey] = value;
          }
        }

        writeFileSync(targetConvFile, JSON.stringify(migrated, null, 2), 'utf-8');
        logger.info('[CharacterManager] Migrated conversations.json (re-keyed with sarkhi: prefix)');
      } catch (err) {
    ...
    const targetPersonaMd = join(sarkhiDir, 'persona.md');
    if (existsSync(defaultPersonaMd) && !existsSync(targetPersonaMd)) {
      cpSync(defaultPersonaMd, targetPersonaMd);
      logger.info('[CharacterManager] Copied persona.md to sarkhi character');
    }

    // Create profile.json
    const profileFile = join(sarkhiDir, 'profile.json');
    ...
    this._save();
    logger.info('[CharacterManager] Migration complete — sarkhi character active');
  }
  ...
  getPersonaMd(id) {
    ...
    // Fallback for sarkhi: use default persona.md
    return null;
  }
