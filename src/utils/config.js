...
const DEFAULTS = {
  ...
  claude_code: {
    ...
    max_turns: 50,
    timeout_seconds: 600,
    workspace_dir: null, // defaults to ~/.sarkhibot/workspaces
...
async function promptForMissing(config) {
  ...
  // Save to ~/.sarkhibot/.env so it persists globally
...
export function loadConfig() {
  // Load .env from CWD first, then from ~/.sarkhibot/
  dotenv.config();
...
/**
 * Save a credential to ~/.sarkhibot/.env and update the live config object.
