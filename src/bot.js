...
import { getHealthSummary, incrementCounter } from './utils/health.js';
import { getStatsSummary, trackMessage, trackCommand } from './utils/analytics.js';
import { addTodo, listTodos, completeTodo, deleteTodo, clearDone, formatTodoList } from './utils/todo.js';
import { addReminder, listReminders, cancelReminder, initReminders, parseReminderTime } from './utils/reminder.js';
import { addNote, listNotes, pinNote, deleteNote } from './utils/notes.js';
import { addSnippet, getSnippet, listSnippets, deleteSnippet } from './utils/snippet.js';
import { createBackup, listBackups } from './utils/backup.js';
import { commandLimiter } from './utils/rate-limiter.js';
...
export function startBot(config, agent, conversationManager, jobManager, automationManager, lifeDeps = {}) {
  ...
  bot.setMyCommands([
    ...
    { command: 'health', description: 'Bot health and system info' },
    { command: 'todo', description: 'Quick todo list management' },
    { command: 'remind', description: 'Set a reminder' },
    { command: 'notes', description: 'Save and manage notes' },
    { command: 'snippet', description: 'Save and recall code snippets' },
    { command: 'weather', description: 'Quick weather lookup' },
    { command: 'translate', description: 'Quick text translation' },
    { command: 'calc', description: 'Quick calculator' },
    { command: 'crypto', description: 'Cryptocurrency prices' },
    { command: 'time', description: 'World clocks and time zones' },
    { command: 'backup', description: 'Backup bot data' },
    ...
    try {
      ...
      // ── Onboarding callbacks ───────────────────────────────────────
        ...
        if (characterManager?.needsOnboarding) {
          // Complete onboarding with sarkhi as default, then build custom
          characterManager.completeOnboarding('sarkhi');
          const sarkhiCtx = agent.loadCharacter('sarkhi');
          rebuildLifeEngine(sarkhiCtx);
        }
  ...
  bot.on('message', async (msg) => {
    ...
    if (text === '/ping') {
      ...
      await bot.editMessageText(`🏓 Pong! Latency: ${elapsed}ms`, { chat_id: chatId, message_id: sent.message_id });
      return;
    }
    ...
    if (text === '/stats') {
      trackCommand('stats');
      await bot.sendMessage(chatId, getStatsSummary(), { parse_mode: 'Markdown' });
      return;
    }
    ...
    if (text === '/health') {
      trackCommand('health');
      await bot.sendMessage(chatId, getHealthSummary(), { parse_mode: 'Markdown' });
      return;
    }
    ...
    if (text === '/todo' || text.startsWith('/todo ')) {
      trackCommand('todo');
      const args = text.slice('/todo'.length).trim();
      if (!args) {
        await bot.sendMessage(chatId, formatTodoList(chatId), { parse_mode: 'Markdown' });
        return;
      }
      if (args.startsWith('add ')) {
        const parts = args.slice(4).trim();
        const priority = parts.startsWith('!') ? 'high' : parts.startsWith('~') ? 'low' : 'normal';
        const taskText = parts.replace(/^[!~]\s*/, '');
        const todo = addTodo(chatId, taskText, priority);
        await bot.sendMessage(chatId, `✅ Added: ${taskText} _(${todo.id.slice(0, 6)})_`, { parse_mode: 'Markdown' });
        return;
      }
      if (args.startsWith('done ')) {
        const id = args.slice(5).trim();
        const todos = listTodos(chatId, { showDone: true });
        const match = todos.find(t => t.id.startsWith(id));
        if (match) { completeTodo(match.id); await bot.sendMessage(chatId, `${match.done ? '⬜ Unchecked' : '✅ Completed'}: ${match.text}`); }
        else await bot.sendMessage(chatId, 'Todo not found.');
        return;
      }
      if (args.startsWith('del ')) {
        ...
        const todos = listTodos(chatId, { showDone: true });
        const match = todos.find(t => t.id.startsWith(id));
        if (match) { deleteTodo(match.id); await bot.sendMessage(chatId, `🗑️ Deleted: ${match.text}`); }
        else await bot.sendMessage(chatId, 'Todo not found.');
        return;
      }
      if (args === 'clear') { clearDone(chatId); await bot.sendMessage(chatId, '🧹 Cleared completed todos.'); return; }
      // Default: add as task
      const todo = addTodo(chatId, args);
      await bot.sendMessage(chatId, `✅ Added: ${args} _(${todo.id.slice(0, 6)})_`, { parse_mode: 'Markdown' });
      return;
    }
    ...
    if (text === '/remind' || text.startsWith('/remind ')) {
      trackCommand('remind');
      const args = text.slice('/remind'.length).trim();
      if (!args) {
        const reminders = listReminders(chatId);
        if (reminders.length === 0) {
          await bot.sendMessage(chatId, '⏰ No active reminders.\nUsage: `/remind in 30 min Check the build`', { parse_mode: 'Markdown' });
        } else {
          const lines = reminders.map(r => {
            const when = new Date(r.triggerAt).toLocaleString();
            return `⏰ ${r.text} — _${when}_ (${r.id.slice(0, 6)})`;
          });
          await bot.sendMessage(chatId, `*Active Reminders:*\n\n${lines.join('\n')}`, { parse_mode: 'Markdown' });
        }
        return;
      }
      if (args.startsWith('cancel ')) {
        ...
        if (match) { cancelReminder(match.id); await bot.sendMessage(chatId, `🗑️ Cancelled reminder: ${match.text}`); }
        else await bot.sendMessage(chatId, 'Reminder not found.');
        return;
      }
      const minutes = parseReminderTime(args);
      if (minutes) {
        const reminderText = args.replace(/in\s+\d+\s*(min|hour|hr|day|week|sec)\w*/i, '').trim() || 'Reminder!';
        const r = addReminder(chatId, reminderText, minutes);
        const when = new Date(r.triggerAt).toLocaleString();
        await bot.sendMessage(chatId, `⏰ Reminder set: ${reminderText}\n🕐 ${when}`, { parse_mode: 'Markdown' });
      } else {
        await bot.sendMessage(chatId, 'Could not parse time. Use: `/remind in 30 min Check the build`', { parse_mode: 'Markdown' });
      }
      return;
    }
    ...
    if (text === '/notes' || text.startsWith('/notes ')) {
      trackCommand('notes');
      const args = text.slice('/notes'.length).trim();
      if (!args) {
        const notes = listNotes(chatId);
        if (notes.length === 0) { await bot.sendMessage(chatId, '📝 No notes yet. Use `/notes add <text>` to create one.', { parse_mode: 'Markdown' }); return; }
        const lines = notes.map(n => `${n.pinned ? '📌' : '📝'} ${n.text.slice(0, 80)} _(${n.id.slice(0, 6)})_`);
        await bot.sendMessage(chatId, `*Your Notes (${notes.length}):*\n\n${lines.join('\n')}`, { parse_mode: 'Markdown' });
        return;
      }
      if (args.startsWith('add ')) {
        const note = addNote(chatId, args.slice(4).trim());
        await bot.sendMessage(chatId, `📝 Note saved _(${note.id.slice(0, 6)})_`, { parse_mode: 'Markdown' });
        return;
      }
      if (args.startsWith('pin ')) {
        ...
        if (match) { pinNote(match.id); await bot.sendMessage(chatId, `📌 ${match.pinned ? 'Unpinned' : 'Pinned'}: ${match.text.slice(0, 50)}`); }
        else await bot.sendMessage(chatId, 'Note not found.');
        return;
      }
      if (args.startsWith('del ')) {
        const id = args.slice(4).trim();
        const notes = listNotes(chatId);
        const match = notes.find(n => n.id.startsWith(id));
        if (match) { deleteNote(match.id); await bot.sendMessage(chatId, `🗑️ Deleted note.`); }
        else await bot.sendMessage(chatId, 'Note not found.');
        return;
      }
      if (args.startsWith('search ')) {
        const query = args.slice(7).trim();
        const results = listNotes(chatId, { search: query });
        if (results.length === 0) { await bot.sendMessage(chatId, `No notes matching "${query}".`); return; }
        const lines = results.map(n => `📝 ${n.text.slice(0, 80)} _(${n.id.slice(0, 6)})_`);
        await bot.sendMessage(chatId, `*Search Results (${results.length}):*\n\n${lines.join('\n')}`, { parse_mode: 'Markdown' });
        return;
      }
      // Default: add as note
      addNote(chatId, args);
    ...
    if (text === '/snippet' || text.startsWith('/snippet ')) {
      trackCommand('snippet');
      const args = text.slice('/snippet'.length).trim();
      if (!args) {
        const snippets = listSnippets(chatId);
        if (snippets.length === 0) { await bot.sendMessage(chatId, '📋 No snippets. Use `/snippet save <name> <code>`', { parse_mode: 'Markdown' }); return; }
        const lines = snippets.map(s => `📋 *${s.name}* (${s.language || 'text'}) _(${s.id.slice(0, 6)})_`);
        await bot.sendMessage(chatId, `*Code Snippets (${snippets.length}):*\n\n${lines.join('\n')}`, { parse_mode: 'Markdown' });
        return;
      }
      if (args.startsWith('save ')) {
        const rest = args.slice(5).trim();
        const spaceIdx = rest.indexOf(' ');
        if (spaceIdx === -1) { await bot.sendMessage(chatId, 'Usage: `/snippet save <name> <code>`', { parse_mode: 'Markdown' }); return; }
        const name = rest.slice(0, spaceIdx);
        const code = rest.slice(spaceIdx + 1);
        addSnippet(chatId, name, code);
        await bot.sendMessage(chatId, `📋 Snippet "${name}" saved.`);
        return;
      }
      if (args.startsWith('get ')) {
        const name = args.slice(4).trim();
        const s = getSnippet(chatId, name);
        if (s) await bot.sendMessage(chatId, `📋 *${s.name}*\n\`\`\`${s.language}\n${s.code}\n\`\`\``, { parse_mode: 'Markdown' });
        else await bot.sendMessage(chatId, `Snippet "${name}" not found.`);
        return;
      }
      if (args.startsWith('del ')) {
        const name = args.slice(4).trim();
        if (deleteSnippet(chatId, name)) await bot.sendMessage(chatId, `🗑️ Deleted snippet "${name}".`);
        else await bot.sendMessage(chatId, `Snippet "${name}" not found.`);
        return;
      }
      // Try to get by name
      const s = getSnippet(chatId, args);
      if (s) await bot.sendMessage(chatId, `📋 *${s.name}*\n\`\`\`${s.language}\n${s.code}\n\`\`\``, { parse_mode: 'Markdown' });
      else await bot.sendMessage(chatId, `Snippet "${args}" not found. Use \`/snippet save <name> <code>\``, { parse_mode: 'Markdown' });
      return;
    }

    // ── /weather — quick weather ──────────────────────────────────
    if (text.startsWith('/weather ')) {
      trackCommand('weather');
      const location = text.slice('/weather '.length).trim();
      if (!location) { await bot.sendMessage(chatId, 'Usage: /weather <city>'); return; }
      text = `What's the weather in ${location}?`;
    }

    // ── /translate — quick translation ────────────────────────────
    if (text.startsWith('/translate ')) {
      trackCommand('translate');
      const args = text.slice('/translate '.length).trim();
      text = `Translate this: ${args}`;
    }
    ...
    // ── /crypto — quick crypto price ─────────────────────────────
    if (text.startsWith('/crypto ')) {
      trackCommand('crypto');
      const coin = text.slice('/crypto '.length).trim();
      text = `What's the current price of ${coin}?`;
    }

    // ── /time — world clocks ─────────────────────────────────────
    if (text === '/time') {
      trackCommand('time');
      text = 'Show me the current time in major cities around the world.';
    }
    ...
    if (text === '/backup') {
      ...
      try {
        ...
        await bot.sendMessage(chatId, `💾 *Backup Created*\n\n📁 ${result.items} items backed up\n📅 ${result.timestamp}`, { parse_mode: 'Markdown' });
      } catch (err) {
    ...
    if (text === '/export') {
      trackCommand('export');
      const history = conversationManager.getHistory(agent._chatKey ? agent._chatKey(chatId) : chatId);
      if (!history?.length) { await bot.sendMessage(chatId, 'No conversation history to export.'); return; }
      const exportText = history.map(m => `[${m.role}]: ${typeof m.content === 'string' ? m.content : JSON.stringify(m.content)}`).join('\n\n---\n\n');
      const { tmpdir } = await import('os');
      const { join: joinPath } = await import('path');
      const { writeFileSync: writeExport } = await import('fs');
      const exportPath = joinPath(tmpdir(), `sarkhibot-export-${Date.now()}.txt`);
      writeExport(exportPath, exportText);
      await bot.sendDocument(chatId, exportPath, { caption: `📄 Conversation export (${history.length} messages)` });
      return;
    }

    // ── /ping — already handled above ────────────────────────────
